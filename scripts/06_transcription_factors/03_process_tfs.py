...
# Script: 03_process_tfs.py
# Description: Classifies PlantTFDB transcription factor predictions and maps TAIR orthology.
    ...
    with open(tf_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if len(parts) >= 3:
                ...
                if tf_family.startswith('CcGene'):
                    continue
                gene_id = re.sub(r'_\d+$', '', raw_gid)
                ath_hit = parts[2].strip() if len(parts) > 2 else '-'
...
if __name__ == '__main__':
    ...
    tf_file = os.path.join(base_dir, 'PlantTFDB.txt')
